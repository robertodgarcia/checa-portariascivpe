...
def enviar_documento(path):
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        print(
            "[Telegram Upload] "
            "TOKEN ou CHAT_ID não configurados."
        )
        return False
...
def url_bis(numero):
    return (
        f"https://www2.pc.pe.gov.br/"
        f"arquivos/BisServ{ANO}/"
        f"bisServ{numero:03d}_{ANO}.pdf"
    )
...
def url_bis_especial(numero):
    return (
        f"https://www2.pc.pe.gov.br/"
        f"arquivos/BisServEspecial{ANO}/"
        f"bisE{ANO}.{numero:02d}.pdf"
    )
...
def montar_relatorio(relatorio):
    ...
    total_bis_especial = len(
        relatorio["bis_especial"]
    )

    total_atualizacoes = (
        total_bis + total_bis_especial
    )

    linhas = []

    # Quando nenhuma atualização for encontrada,
    # não será criada nem enviada mensagem ao Telegram.
    if total_atualizacoes == 0:
        print(
            "[INFO] Nenhuma atualização encontrada. "
            "Mensagem não enviada ao Telegram."
        )
        return None

    linhas.append(
        "ATUALIZAÇÃO IDENTIFICADA NO MONITOR BIS"
    )
    linhas.append("")
    ...
    linhas.append(
        f"Foram encontradas "
        f"{total_atualizacoes} "
        "atualização(ões) nova(s)."
    )
    ...
    if relatorio["bis"]:
        ...
        for item in relatorio["bis"]:
            if item["status"] == "ERRO_NO_DOWNLOAD":
                ...
                linhas.append(
                    f"- {item['nome']}: "
                    "atualização encontrada e "
                    "palavra(s)-chave localizada(s): "
                    f"{', '.join(item['palavras'])}."
                )
                ...
                linhas.append(
                    ...
                    "sem ocorrência das palavras-chave "
                    "monitoradas."
                )
...
def enviar_documentos_com_palavra_chave(
    relatorio
):
    enviados = 0
    ...
    print(
        "[INFO] documentos enviados "
        f"ao Telegram: {enviados}"
    )
...
def main():
    ...
    print(
        f"[INFO] PALAVRAS_CHAVE="
        f"{PALAVRAS_CHAVE}"
    )
    print(
        f"[INFO] TELEGRAM_CHAT_ID="
        f"{TELEGRAM_CHAT_ID}"
    )
    ...
    print(
        f"[INFO] Estado final={estado}"
    )

    texto_relatorio = montar_relatorio(
        relatorio
    )

    # Só envia a mensagem quando houver
    # alguma atualização.
    if texto_relatorio:
        enviar_telegram(texto_relatorio)

    enviar_documentos_com_palavra_chave(
        relatorio
    )
